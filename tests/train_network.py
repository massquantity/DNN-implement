...
if __name__ == "__main__":
    ...
    dnn = Network_mini_batch(sizes=[784, 300, 200, 10], activation="selu", dropout_rate=0.0,  # dropout alpha
                             weight_initializer="he")
    optimizer = Adam(lr=1e-3, batch_size=64)
#    optimizer = Sgd(lr=1e-3, batch_size=64)
    start_time = time.time()
    train_DNN_minibatch(X_train, y_train, 150, optimizer, 64, dnn, X_test, y_test,
                        early_stopping=True, patience=20, metrics="accuracy", evaluate=True)  # lr_decay_rate=0.95
